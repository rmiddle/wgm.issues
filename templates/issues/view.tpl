...
<input type="hidden" name="context_id" value="wgm.contexts.githubissue">
...
<table cellpadding="1" cellspacing="0" border="0" width="100%" class="worklistBody">
	...
	{* Column Data *}
	{$object_watchers = DAO_ContextLink::getContextLinks('wgm.contexts.githubissue', array_keys($data), CerberusContexts::CONTEXT_WORKER)}
	...
	<tbody style="cursor:pointer;">
		<tr class="{$tableRowClass}">
			<td align="center" rowspan="2" nowrap="nowrap" style="padding:5px;">
				{include file="devblocks:cerberusweb.core::internal/watchers/context_follow_button.tpl" context='wgm.contexts.githubissue' context_id=$result.g_id}
			</td>
			<td colspan="{$smarty.foreach.headers.total}">
				<input type="checkbox" name="row_id[]" value="{$result.g_id}" style="display:none;">
				{if $result.g_state == 'closed'}
					<span class="cerb-sprite2 sprite-tick-circle-frame-gray"></span>
				{/if}
				<a href="{devblocks_url}c=github&a=issue&id={$result.g_id}-{$result.g_title|devblocks_permalink}{/devblocks_url}" class="subject">{$result.g_title}</a> 
				<button type="button" class="peek" style="visibility:hidden;padding:1px;margin:0px 5px;" onclick="genericAjaxPopup('peek','c=github&a=showEntry&id={$result.g_id}&view_id={$view->id}',null,false,'550');"><span class="cerb-sprite2 sprite-document-search-result" style="margin-left:2px" title="{$translate->_('views.peek')}"></span></button>
			</td>
		</tr>
		<tr class="{$tableRowClass}">
		{foreach from=$view->view_columns item=column name=columns}
			...
			{elseif $column=="g_created_date" || $column == "g_updated_date"}
				<td title="{$result.$column|devblocks_date}">
					{if !empty($result.$column)}
						{$result.$column|devblocks_prettytime}&nbsp;
					{/if}
				</td>
			{elseif $column=="g_is_outgoing"}
				<td>
					{if $result.$column}
						Outgoing
					{else}
			...
			{elseif $column=="g_state"}
				<td>
					{if $result.$column == "open"}
						{'status.open'|devblocks_translate|capitalize}
					{elseif $result.$column == "closed"}
